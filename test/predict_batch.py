...
def main(args):
    ...
    sequences = [s[1] for s in results] ## (NN)
    
    positions = torch.cat(positions, dim=0).detach().cpu().numpy()
    probabilities = torch.cat(sequences, dim=0).detach().cpu().numpy() #(NN)
    torch.save(probabilities, "sampled_seq_gvp.pt") # (NN)
